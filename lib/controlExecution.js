    ...
    services.forEach(function(entry) {
        ...
        if ("capabilities" in entry) {
            if ("maps" in entry.capabilities[0]) {
              if (!(typeof(entry.capabilities[0].maps)) === "object") {
                  console.log("The value of 'maps' has to be a object");
                  process.exit();
              }

              if (!("defaultWidth" in entry.capabilities[0].maps)) {
                  console.log("The capabilities of maps do not contain width.");
                  process.exit();
              }

              if (!(typeof(entry.capabilities[0].maps.defaultWidth)) === "number" || entry.capabilities[0].maps.width <= 0) {
                  console.log("The value of 'width' has to be a number > 0");
                  process.exit();
              }

              if (!("defaultHeight" in entry.capabilities[0].maps)) {
                  console.log("The capabilities of maps do not contain defaultHeight.");
                  process.exit();
              }

              if (!(typeof(entry.capabilities[0].maps.defaultHeight)) === "number" || entry.capabilities[0].maps.width <= 0) {
                  console.log("The value of 'height' has to be a number > 0");
                  process.exit();
              }

              if (!("defaultFormat" in entry.capabilities[0].maps)) {
            ...
            if ("features" in entry.capabilities[0]) {
              if (!("defaultFormat" in entry.capabilities[0].features)) {
