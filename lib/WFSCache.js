...
var updateServices = function(servicesDescription) {
  console.log("At function updateServices() in WFSCache");
    ...
    for (var i = 0; i < servicesDescription.length; i++) {
        ...
        var url = processJSON.getGetCapabilitiesURLWFS(servicesDescription, id); // constructs the URL to a given SerivceID from the ServiceDescriptions
        console.log("updateServices WFSCache url: +++++++++++++++++++++++++++++++++: " + url);
        if (url) {
          serviceURLs.push({
              id: id,
              url: url
