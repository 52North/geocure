...
var updateServices = function() {
  if(!services){
    return next(new error.InternalServereError());
  }
    ...
    for (var i = 0; i < servicesDescription.length; i++) {
        ...
        var url = helpingFunctions.getGetCapabilitiesURL(servicesDescription, id); // constructs the URL to a given SerivceID from the ServiceDescriptions
        serviceURLs.push({
            id: id,
            url: url
