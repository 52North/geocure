...
function errorHandling(services, id, serviceCache, crs) {
    ...
    if (!ServiceGetCapabilities) {
        var message = new error.InternalServerError("The capabilities are currently not available. Please try again later.");
        return message;
    }
...
function checkLeyers(serviceCache, id, layer) {
    var serviceGetCapabilities = serviceCache.getServices()[id];
    console.log("1");
