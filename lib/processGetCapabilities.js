...
function getAllFeaturesInfo(WFSCache, req) {
    ...
    for (var i = 0; i < features.length; i++) {
        ...
        var value = req.fullUrl + '/' + features[i].name + "/data";
        fo[property] = value;
    }
...
function getAllLayersAsString(getCapabilities) {
    ...
    if (layerObject.code) {
        ...
        for (var propName in layerObject) {
            if (layerObject.hasOwnProperty(propName)) {
                var propValue = layerObject[propName];
                res.push(propValue.substring(1,propValue.length-7));
            }
        }
