...
function constructServiceInfo(getCapabilities, req) {
  ...
  if (getCapabilities.capabilities.maps) {
    response.capabilities.push(req.fullUrl + "/maps");
  }


  if (getCapabilities.capabilities.features) {
...
console.log("At constructMapInfo()");
  ...
  let crs = getCRS(getCapabilities);
  mapinfo.addCrs(crs);
...
**/
function getAllFeaturesInfo(WFSCache, req) {
    ...
    for (var i = 0; i < featureTypeList.length; i++) {
        features.push({
            ...
            name: String(featureTypeList[i].name).substring(String(featureTypeList[i].name).indexOf('}') + 1),
    ...
    for (var i = 0; i < features.length; i++) {
      ...
      var aFeature = {};
      aFeature["id"] = features[i].name;
      aFeature["title"] = features[i].title;
      aFeature["href"] = req.fullUrl + '/' + features[i].name + "/data";
...
function getFeatureInfo(services, WFSCache, req) {
    ...
    return  getAllFeaturesInfo(WFSCache, req); // Accessing the layer-section of the GetCapabilitie-Json
...
function getLayerInfo(getCapabilities, requestURL) {
    ...
    var layersObjects = []; // Object to be filled
    if (!getCapabilities) {
        return new error.InternalServerError("The getCapabilities are not accessible");
    }
    if (!getCapabilities.WMS_Capabilities) {
    ...
    if ("name" in layer) { // if TRUE => Onle this layer exists and no further layer in a folowing array.
        ...
        for (var i = 0; i < layer.layer.length; i++) {
          ...
          layerObj["id"] = layer.layer[i].name;
          layerObj["title"] = layer.layer[i].title;
          layerObj["href"] = requestURL + '/' + layerObj["id"] + "/render";


          layersObjects.push(layerObj);
        }
    }
...
Description: Takes getCapabilities and returns all id of offered layers as a string.
**/
function getAllLayersAsString(getCapabilities) {
    ...
    if (layerObject.code) {
        console.log("Finishing getAllLayersAsString() with error");
        return layerObject;
    } else {
        console.log("In ELSE");
        let res = [];

        for (var i = 0; i < layerObject.length; i++) {
          res.push(layerObject[i].id);
        }
        console.log("Finishing getAllLayersAsString() without error");
        return res.toString();
    }
}
...
module.exports = {
    ...
    getAllFeaturesInfo: getAllFeaturesInfo
}
