...
server.get({
    ...
    for (var i = 0; i < services.length; i++) {
        ...
        if (obj.id == req.params.id) {
            // By constructing the output. Way to avoid redudant information about services in different files.
            var serviceInfo = new ServiceInformation(obj.id, obj.label, obj.description, obj.href, obj.capabilities);
...
server.get({
    ...
    if (checkedRequest.idFound) {
        if ("maps" in checkedRequest.object.capabilities[0]) {
            request.get(checkedRequest.object.href + checkedRequest.object.capabilities[0].maps).pipe(res);
        }
    }
...
server.get({
    ...
    if (checkedRequest.idFound) {
        if ("features" in checkedRequest.object.capabilities[0]) {
            request.get(checkedRequest.object.href + checkedRequest.object.capabilities[0].features).pipe(res);
        }
    }
