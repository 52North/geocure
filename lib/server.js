...
var request = require('request'); // Delete?
...
server.get({
    ...
    //ERROR-HANDLING:_ req-parmas.id was checked at the beginning of this endpoint
    // Worstcase: GetCapabilities do not exist.
    var ServiceGetCapabilities = serviceCache.getServices()[req.params.id]; // Get JSON-GetCapabilities to requestet
    // ERROR-HANDLING
    console.log("Does ServiceGetCapabilities exists?");
    if (!ServiceGetCapabilities) {
        return next(new error.InternalServerError("No GetCapabilities for service with id " + req.params.id));
    }
    console.log("Yes, ServiceGetCapabilities exists");

    // ERROR-HANDLING: Checked after starting the server
    var baseURL = helpingFunctions.getBaseURL(services, req.params.id) + "wms?";
    console.log(baseURL); // For development
    var service = "service=wms";


    // ERROR-HANDLING
    console.log("Does ServiceGetCapabilities.WMS_Capabilities.version exists?");
    if (!ServiceGetCapabilities.WMS_Capabilities.version) {
        return next(new error.InternalServerError("Version of the Service not available"));
    }
    console.log("Yes, ServiceGetCapabilities.WMS_Capabilities.version exists");
    var version = "version=" + ServiceGetCapabilities.WMS_Capabilities.version;

    console.log(version); // For development
    ...
    // ERROR-HANDLING
    console.log("Are there any layers to show?");
    if (!helpingFunctions.getAllLayersRequest(ServiceGetCapabilities.WMS_Capabilities.capability.layer)) {
        return next(new error.InternalServerError("No Layer to show"));
    }
    console.log("Yes, there are layer to show");
    var layers = "layers=" + helpingFunctions.getAllLayersRequest(ServiceGetCapabilities.WMS_Capabilities.capability.layer);
    console.log(layers); // For development


    var json = serviceCache.getServices()[req.params.id]; // Get JSON-GetCapabilities to requestet
    // ERROR-HANDLING
    console.log("Does the json-file for the requested service exist?");
    if (!serviceCache.getServices()[req.params.id]) {
        return next(new error.InternalServerError("required json-file is not available. Please wait and reload again."));
    }
    console.log("Yes, the json-file for the requested service exists");
    ...
    // ERROR-HANDLING
    console.log("Does the bbox for request the exists?");
    if (!helpingFunctions.getRequestBBOX(json)) {  // json is have to be checked previously
        return next(new error.InternalServerError("Required information about global bbox of the service is not available"));
    }
    console.log("Yes, the bbox for request the exists");

    var bboxANDrefsys = "bbox=" + helpingFunctions.getRequestBBOX(json);
    var width = "width=" + 1330; // By default
    var height = "height=" + 944 // By default
    var format = "format=image/png"; // By default
...
r
    ...
    var requestURL = (baseURL + service + "&" + version + "&" + request + "&" + layers + "&" + bboxANDrefsys + "&" + width + "&" + height + "&" + format);
