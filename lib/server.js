...
server.get({
    ...
    // Description: Takes the GetCapabilities of an OGC-WMS and returns a JSON-Representation with specified content.
        ...
        var json = serviceCache.getServices()[req.params.id];
        if(!json){
...
server.get({
    ...
    //         var url = NULL;
    //         resolve(helpingFunctions.getGetCapabilitiesURL(serviceDescription, id));
    //     });
    // }
    //
    // getURL(services, "COLABIS_REST_TEST").then(function(response) {
    //     res.send("hi");
    // });
    ...
    console.log("hii We are starting!");
    getURL(services, "COLABIS_REST_TEST")
    .then(function(url) {
      console.log("x comes");
      helpingFunctions.unmarshallXML(url);
      console.log("here at var v");
      console.log(v);
      console.log("after v");
    }).then(function(){
      res.send("done");
