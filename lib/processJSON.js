...
function getGetCapabilitiesURL(objectArray, serviceId) {
    ...
    console.log(checkedRequest.baseURLService + "wms?request=GetCapabilities&service=wms");
    return checkedRequest.capabilities[0].maps ? (checkedRequest.baseURLService + "wms?request=GetCapabilities&service=wms") : null;
}

/**
Signature: getGetCapabilitiesURLWFS : Array, String -> String
...
function getGetCapabilitiesURLWFS(objectArray, serviceId) {
    console.log("At function getCapabilitiesUrlWFS()");
    var checkedRequest = getKeyValueObject(objectArray, "id", serviceId);
    return checkedRequest.capabilities[0].features ? (checkedRequest.baseURLService + "wfs?request=GetCapabilities&service=wfs&version=1.1.0") : null;
}
...
module.exports = {
  ...
  getGetCapabilitiesURLWFS: getGetCapabilitiesURLWFS
}
