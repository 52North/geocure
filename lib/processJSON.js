...
function getGetCapabilitiesURL(objectArray, serviceId) {
    ...
    if (checkedRequest.capabilities[0].maps) {
       getCapabilitiesUrl = checkedRequest.baseURLService + "wms?request=GetCapabilities&service=wms";
    }
...
function getGetCapabilitiesURLWFS(objectArray, serviceId) {
    ...
    if (checkedRequest.capabilities[0].features) {
       getCapabilitiesUrl = checkedRequest.baseURLService + "wfs?request=GetCapabilities&service=wfs&version=1.1.0";
    }
    console.log("Finishing getCapabilitiesUrlWFS()");
    return getCapabilitiesUrl;
}
