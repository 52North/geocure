...
function checkQueryFeatures(req, services, WFSCache) {
    ...
    if (req.params.features) {
        ...
        var notValid = checkFeatures(req.params.feature); // Are they valid?
        console.log("Features not valid?: " + notValid);
        if (notValid) {
            return new error.BadRequestError("At least one requested feature is not valid");
    ...
    if (params.crs) {
        ...
        if(valid) {
            ...
            var message = new error.BadRequestError("The requested CRS is not supported.");
            requestParams.crs = {
                "error": message
            };
        }
...
function checkFeatures (feature) {
  var featureArray = String(feature).splot(",");
