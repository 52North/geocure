...
**/
function checkExtent(params, maxExtent) {
...
function getMaxExtentWMS (serviceCache, params) {
...
function getMapRequestURL(params, serviceCache, services) {
    ...
    var width = !checkedQuery.width ? getDefaultWidth(params, services) : checkedQuery.width;

    var height = !checkedQuery.height ? getDefaultHeight(params, services) : checkedQuery.height;

    var format = !checkedQuery.format ? getDefaultFormat(params, services) : checkedQuery.format;
...
function getDefaultWidth(params, services) {
  for (let i = 0; i < services.length; i++) {
    if (params.id == services[i].id) {
...
function getDefaultHeight(params, services) {
  for (let i = 0; i < services.length; i++) {
    if (params.id == services[i].id) {
...
function getDefaultFormat(params, services) {
  for (let i = 0; i < services.length; i++) {
    if (params.id == services[i].id) {
