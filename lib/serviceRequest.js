...
function getMaxExtentWFS (services, serviceCache, req) {
console.log("At function getMaxExtentWFS");
  ...
  var info = processGetCapabilities.getFeatureInfo(services, serviceCache, req);


  var maxExtent = {
    crs : info.crs[0].crs,
    minx: info.crs[0].minx,
    miny: info.crs[0].miny,
    maxx: info.crs[0].maxx,
    maxy: info.crs[0].maxy
  }

  console.log("MaxExtent WFS minx : " + info.crs[0].minx);
...
function checkExtent(req, maxExtent) {
  console.log("At function checkExtent()");
    var requestedCrs = null
    console.log("maxExtent crs: " + maxExtent);
    ...
    console.log("maxExtent.miny " + maxExtent.miny);
    console.log("givenCoordinates.miny " + givenCoordinates.miny);

    console.log("maxExtent.maxx " + maxExtent.maxx);
    console.log("givenCoordinates.maxx " + givenCoordinates.maxx);

    console.log("maxExtent.maxy " + maxExtent.maxy);
    console.log("givenCoordinates.maxy " + givenCoordinates.maxy);

    console.log("maxExtent.minx > givenCoordinates.minx = " + (maxExtent.minx > givenCoordinates.minx));
    console.log("maxExtent.miny > givenCoordinates.miny = " + (maxExtent.miny > givenCoordinates.miny));
    console.log("maxExtent.maxx < givenCoordinates.maxx = " + (maxExtent.maxx < givenCoordinates.maxx));
    console.log("maxExtent.maxy < givenCoordinates.maxy = " + (maxExtent.maxy < givenCoordinates.maxy));
    ...
    if (maxExtent.minx > givenCoordinates.minx || maxExtent.miny > givenCoordinates.miny || maxExtent.maxx < givenCoordinates.maxx || maxExtent.maxy < givenCoordinates.maxy) {
        ...
        var message = new error.BadRequestError("The requested coordinates are not in the extent of the globale bbox. Given minx = " + givenCoordinates.minx + ". Minimum minx = " + maxExtent.minx + ". Given miny = " + givenCoordinates.miny + ". Minimum miny = " + maxExtent.miny + ". Given maxx = " + givenCoordinates.maxx + ". Maximum maxx = " + maxExtent.maxx + ". Given maxy = " + givenCoordinates.maxy + ". Maximum maxy = " + maxExtent.maxy);
        console.log("Finishing checkExtent() with an error");
        return {
            "ok": false,
            "error": {
                "error": message
    ...
    console.log("Finishing checkExtent() without an error");
    return {
        "ok": true,
...
function checkBbox(req, maxExtent) {
  console.log("At function checkBbox()");
    // Valid format? We only want to use numbers
    if (isNaN(req.params.minx)) {
      console.log("MINX IS NOT A Number!!!!!!!!");
    ...
    console.log("Finishing function checkBbox()");
...
function checkQueryFeatures(req, services, WFSCache) {
    ...
    if (req.params.minx || req.params.miny || req.params.maxx || req.params.maxy) { // Are elements of a bbox requested?
        if (req.params.minx && req.params.miny && req.params.maxx && req.params.maxy) { // Are they complete?
            var bboxExtentionCheck = checkBbox(req, getMaxExtentWFS(services, WFSCache, req)); // Is the given bbox valid? getting bbox or error
            console.log("bboxExtentionCheck.error ? : " + bboxExtentionCheck.error);
            if (bboxExtentionCheck.error) {
              return bboxExtentionCheck.error;
            }
            requestParams["bbox"] = checkBbox(req, getMaxExtentWFS(services, WFSCache, req)); // Is the given bbox valid? getting bbox or error
        } else {
    ...
    if (req.params.format) {
      console.log("Check the format")
        ...
        var requestedFormat = req.params.format; //String(req.params.format).replace("; ","placemark"); // mark intended whitespaces
        ...
        for (let i = 0; i < validFormats.length; i++) {
          if (validFormats[i] === decodeURI(String(requestedFormat))) {
            notValid = false;
            break;
          }
        }

        console.log("Is the requested format NOTvalid?: " + notValid);
        ...
        if (notValid) {
          ...
          console.log("stored formats are: " + requestParams.format);
        }
    }
...
function getFeatureRequestURL(req, WFSCache, services) {
  ...
  var outputformat = "outputFormat=" + (checkedQuery.format? checkedQuery.format : getDefaultFormatFeatures(req, services));
  console.log("checkedQuery.format? " + checkedQuery.format);
  console.log("Outputformat " + outputformat);

  var requestURL = baseURL + service + "&" + version + "&" + request + "&" + "typeName=" + featureID + "&" +  outputformat;
  if (checkedQuery.bbox) {
    console.log("bbox coordinates are: ");
    console.log(checkedQuery.bbox.toString())
    requestURL = requestURL + "&bbox=" + checkedQuery.bbox.minx + "," + checkedQuery.bbox.miny + "," + checkedQuery.bbox.maxx + "," + checkedQuery.bbox.maxy;
  }
  if (checkedQuery.crs) {
    console.log("bbox coordinates are: ");
    console.log(checkedQuery.bbox.toString())
    requestURL = requestURL + "&srsName=" + checkedQuery.crs;
  }
...
function checkQuery(req, services, serviceChache) {
    ...
    if (req.params.format) {
      console.log("USER INPUT FORMAT = " + req.params.format);
        ...
        // Problem is: There are formats, which do have a "+" - sign and/ore a "; " in their name.
        // "+" is a whitespace in URLs. So we have to replace unintended whitespaces
        var format = req.params.format; //String(req.params.format).replace("; ","placemark"); // mark intended whitespaces
